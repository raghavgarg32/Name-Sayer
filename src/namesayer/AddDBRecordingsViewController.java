...
public class AddDBRecordingsViewController extends SideButtons {
	...
	public void handleAddRecordingButton() throws IOException {
		...
		if ((userName.getText().matches("[a-zA-Z]+"))
				&& !(DataBaseController.getNamesWithoutNumbers().contains(userName.getText().toLowerCase()))) {
			recordingForDB = true;
			...
			Alert alert = new Alert(Alert.AlertType.NONE,
					"Please only enter name with characters A-Z a-z and a name that doesn't exist.", ButtonType.OK);
			alert.showAndWait();
