...
public class RateViewController extends SideButtons implements Initializable {
	...
	/**
	 * If the user gives the database recording a bad review this will output it to
	 * a text file
	 */
	@FXML
	public void handleBadButton() {
		...
		BashCommandWorker creationDirectoryWorker = new BashCommandWorker(
				"badRecordingMessage='" + HomeViewController.getNamesHashMap().get(currentName) + "'\n" + "\n"
						+ "if ! grep -qF \"$badRecordingMessage\" BadRecordingList.txt ; then "
						+ "echo \"$badRecordingMessage\" >> BadRecordingList.txt ; " + "fi\n"
						+ "if ! grep -qF \"$badRecordingMessage recording for " + selectionName
						+ " has a bad quality\" ./Database/" + selectionName + "/Ratings.txt ; then "
						+ "echo \"$badRecordingMessage recording for " + selectionName
						+ " has a bad quality\" >> ./Database/" + selectionName + "/Ratings.txt ; " + "fi");

		Alerts.show(
				"This recording has been rated as bad, in the future we will try to provide you with better recordings",
				ButtonType.OK, null);
	...
	public void handleGoodButton() {
		...
		BashCommandWorker creationDirectoryWorker = new BashCommandWorker(
				"sed -i '/" + HomeViewController.getNamesHashMap().get(currentName) + "/d' ./BadRecordingList.txt; \n"
						+ "sed -i '/" + HomeViewController.getNamesHashMap().get(currentName) + " recording for "
						+ selectionName + " has a bad quality/d' ./Database/" + selectionName + "/Ratings.txt ;");
