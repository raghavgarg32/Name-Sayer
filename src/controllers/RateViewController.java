...
public class RateViewController extends SideButtons implements Initializable {
	...
	/**
	 * This sets up the rating list view with the appropiate names which are all of the names in the user selected name
	 * and also the head name
	 */
	public void setUp(){
		
		name.setText(PracticeViewController.getCurrentName(false));
		String[] individualNames = PracticeViewController.getCurrentName(false).split("[-\\s]");

		ObservableList<String> individualNamesObList = FXCollections.observableArrayList(individualNames);
		String tempCurrentName = PracticeViewController.getCurrentName(false);
