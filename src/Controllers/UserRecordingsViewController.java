...
public class UserRecordingsViewController extends SideButtons {
    ...
    public void handlePlayUserRecordingButton() throws IOException {
        // Check if an item has been selected in userCreations
    ...
    public void handleDeleteButton() {
    	if (userCreations.getSelectionModel().isEmpty()) {
    		...
    		try {
				Files.deleteIfExists(Paths.get(System.getProperty("user.dir")+ "/User-Recordings/"+userCreations.getSelectionModel().getSelectedItem()));
				settingUserListView();
				System.out.println(Paths.get(System.getProperty("user.dir")+ "/User-Recordings/"+userCreations.getSelectionModel().getSelectedItem()));
			} catch (IOException e) {
				e.printStackTrace();
			}
    	}
